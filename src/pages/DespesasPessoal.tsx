...
import { Plus, Search, Loader2, Trash2, DollarSign, Lock } from "lucide-react";
...
export default function DespesasPessoal() {
  ...
  const { data: despesas, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("despesas_pessoal").select("*").order("data_vencimento", { ascending: false });
                      ...
                      <tr key={d.id} className="border-b border-border/50 hover:bg-surface-hover transition-colors">
                        ...
                        <td className="p-3">
                          <div className="flex gap-1">
                            ...
                            {role === "admin" && (
                              <button onClick={() => { if (confirm("Excluir?")) deleteMutation.mutate(d.id); }} className="p-1.5 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                            )}
                          </div>
                        </td>
                      </tr>
